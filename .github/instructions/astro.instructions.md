...
- Define a `Props` interface in frontmatter for every reusable component and document the interface with a short TSDoc comment describing the component contract.
- Document each non-obvious prop in the `Props` interface with a property comment. Keep the contract accurate when props change.
...
- Use comments to explain intent or an unusual Astro/build-time constraint, not to narrate markup that is already self-explanatory.
