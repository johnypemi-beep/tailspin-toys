...
The home page supports selecting one or more categories and one publisher. Category
selections match any selected category, while a publisher selection combines with
categories to narrow the results. Filter selections are stored in the page URL,
so a filtered catalog view can be shared or restored after a refresh.
...
[`.github/copilot-instructions.md`](.github/copilot-instructions.md). The focused instruction files cover
[Astro components](.github/instructions/astro.instructions.md),
[UI components](.github/instructions/ui.instructions.md), and the
[Drizzle/SQLite data layer](.github/instructions/drizzle.instructions.md). In particular, comments should
explain intent rather than restate code, exported data-layer functions require TSDoc/JSDoc for their
parameters and return values, and reusable Astro components must document their `Props` contract.
...
The shared **Issue Triage Board** canvas (`.github/extensions/issue-triage-board/`) fetches open GitHub issues with the GitHub CLI, ranks the three most likely to need attention, and places the remaining issues below them. Use an issue card's **Add to current context** button to send a focused work prompt to the current session.
